...
if (isset($_POST["answer"])) {
    ...
    if ($answer === 'yes') {
        ...
        header('location:read-previous.php');
    } else {
        header('location:../../error.php');
    }
}

$sql = "SELECT `status` FROM `message` WHERE status='0' AND username='$username'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo '<audio src="sounds/message.mp3" autoplay></audio>';
} else {
    header('location:no-message.php');
}
$conn->close();
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <meta charset="UTF-8">
        <meta http-equiv="refresh" content="15;url=check.php" />
